...
function formatData(tasks){
    var out = "<table class='table table-bordered table-hover table-info'>" +
        "<tr>" +
        "<th style='font-weight: bold;'>Task</th><th>Edit Tasks</th><th>Complete Tasks</th>" +
        "</tr>";
    for (let i in tasks) {
        out += "<tr><td>"+tasks[i].name+"<br>"+tasks[i].description+"</td>" +
               "<td><a href='../edit-task/edit-task.html?id="+tasks[i].id+"'><button class='btn btn-light'>Edit</button></a></td>" +
